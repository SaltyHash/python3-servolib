"""
Library for controlling digital servos using the `LewanSoul Bus Servo Communication Protocol
...
class LewanSoulServoBus:
    """
    Controls digital servos on a half-duplex serial bus using the LewanSoul Bus Servo Communication Protocol.

    Example usage::

        # Create a new LewanSoulServoBus instance using a "with ..." statement,
        # which by default will power on all the servos.
        with LewanSoulServoBus(serial.Serial(...)) as servos:
            # Move servo 2 to the 120 degree position over 1 second
            servos.move_time_write(2, 120, 1)
    ...
    def _send_packet(self, servo_id: int, command: int, parameters: Union[bytearray, bytes] = None) -> None:
        # The servo command packet format is as follows:
        #
        ...
        if servo_id < 0 or servo_id > BROADCAST_ID:
            raise LewanSoulServoBusException(f'servo_id must be in range [0, {BROADCAST_ID}]; got {servo_id}.')
        if command < 0 or command > 255:
            raise LewanSoulServoBusException(f'command must be in range [0, 255]; got {command}.')
        ...
        with self._conn_lock:
            # Send the packet
            byte_count = self._conn.write(servo_packet)
            self._conn.flush()

            if self.discard_echo:
                self._conn.read(byte_count)

    def _receive_packet(self) -> _ServoPacket:
        with self._conn_lock:
            header = self._conn.read(2)
            if header != _PACKET_HEADER:
                raise LewanSoulServoBusException(
                    rf'Expected header {repr(_PACKET_HEADER)}; received header {repr(header)}.')
    ...
    def _send_and_receive_packet(
        ...
        # Make sure we're not sending this to the broadcast ID
        if servo_id == BROADCAST_ID:
            raise ValueError(f'servo_id cannot be the broadcast ID ({BROADCAST_ID}).')

        # Send the packet and receive the response packet
        with self._conn_lock:
    ...
    def mode_read(self, servo_id: int) -> Tuple[str, Optional[int]]:
        ...
        if mode == 0:
            ...
            raise LewanSoulServoBusException(f'Received unknown mode: {mode}')
...
class LewanSoulServoBusException(Exception):
    pass
...
def _get_servo_ids() -> List[int]:
    servo_ids = input('Enter servo IDs (comma-separated): ')
    servo_ids = [int(servo_id.strip()) for servo_id in servo_ids.split(',')]
    return servo_ids
...
def watch_servo_state(servos: LewanSoulServoBus) -> None:
    ...
    try:
        ...
        servo_ids = _get_servo_ids()
        if not servo_ids:
            print('No servo IDs entered; exiting.')
            return
...
def test(servos: LewanSoulServoBus) -> int:
    ...
    servo_ids = _get_servo_ids()
    if not servo_ids:
        print('No servo IDs listed to test; exiting.')
        return 1
